...
const ChatBox: React.FC = () => {
  ...
  const renderComboCardFromInfo = (comboInfo: { name: string; price?: string }): React.ReactNode | null => {
    ...
    // Loại bỏ prefix "combo" nếu có để tìm trong cache
    const cleanComboName = comboName.replace(/^combo\s+/i, '').trim();
    
    console.log('🔍 [Combo Card] Searching for combo:', {
      comboName,
      cleanComboName,
      cacheSize: combosCache.size,
      cacheKeys: Array.from(combosCache.keys()).slice(0, 5)
    });
    ...
    if (comboName) {
      ...
      if (combo) console.log('✅ [Combo Card] Found with full name:', combo.name, 'hasImage:', !!combo.image);
      
      // 2. Tìm với tên không có "combo" prefix
      if (!combo && cleanComboName) {
        combo = findComboInCache(cleanComboName);
        if (combo) console.log('✅ [Combo Card] Found with clean name:', combo.name, 'hasImage:', !!combo.image);
      }
      
      // 3. Tìm với "combo" prefix
      if (!combo && cleanComboName) {
        combo = findComboInCache(`combo ${cleanComboName}`);
        if (combo) console.log('✅ [Combo Card] Found with "combo" prefix:', combo.name, 'hasImage:', !!combo.image);
      }
      ...
      if (!combo && cleanComboName) {
        ...
        if (combo) console.log('✅ [Combo Card] Found without tones:', combo.name, 'hasImage:', !!combo.image);
      }
      ...
      if (!combo) {
        ...
        for (const [key, cachedCombo] of combosCache.entries()) {
          ...
          if (normalizedKey.includes(normalizedSearch) || 
            ...
            combo = cachedCombo;
            console.log('✅ [Combo Card] Found with fuzzy match:', combo.name, 'hasImage:', !!combo.image, 'matchedKey:', key);
            break;
          }
        }
      }
      
      // 6. Nếu vẫn không tìm thấy, thử tìm với tên được normalize từ comboName
      if (!combo) {
        const normalizedComboName = normalizeText(comboName);
        combo = findComboInCache(normalizedComboName);
        if (combo) console.log('✅ [Combo Card] Found with normalized name:', combo.name, 'hasImage:', !!combo.image);
      }
    }
    
    if (!combo) {
      console.warn('⚠️ [Combo Card] Combo not found in cache:', comboName);
    }
    
    // ✅ Nếu không tìm thấy combo trong cache, thử search API để tìm combo
    if (!combo && (cleanComboName || comboName) && (cleanComboName?.length >= 2 || comboName.length >= 2)) {
      console.log('🔍 [Combo Card] Searching API for:', cleanComboName || comboName);
      // Search API với cả tên có và không có "combo" prefix
      const searchTerms = cleanComboName ? [cleanComboName, comboName] : [comboName];
      
      // Search với từng term (không đợi, sẽ update cache sau)
      searchTerms.forEach(searchTerm => {
        if (searchTerm && searchTerm.length >= 2) {
          fetch(`${API_URL}/api/combos?search=${encodeURIComponent(searchTerm)}&limit=10`)
            .then(res => res.json())
            .then(data => {
              const combos = data.data?.items || data.data?.combos || data.combos || data.data || [];
              console.log('📦 [Combo Card] API search result:', combos.length, 'combos found');
              if (Array.isArray(combos) && combos.length > 0) {
                // Tìm combo có tên khớp nhất
                const normalizedSearch = normalizeText(searchTerm);
                const foundCombo = combos.find((c: any) => {
                  const cName = normalizeText(c.name || '');
                  return cName.includes(normalizedSearch) || normalizedSearch.includes(cName);
                }) || combos[0];
                
                if (foundCombo) {
                  console.log('✅ [Combo Card] Found combo from API:', foundCombo.name, 'hasImage:', !!(foundCombo.image || foundCombo.imagePath || foundCombo.thumbnail));
                  const comboData = {
                    id: foundCombo.id || foundCombo._id,
                    name: foundCombo.name,
                    image: foundCombo.image || foundCombo.imagePath || foundCombo.thumbnail || foundCombo.images?.[0] || null,
                    price: foundCombo.price ? Number(foundCombo.price) : undefined,
                    slug: `${removeVietnameseTones(foundCombo.name)}-${foundCombo.id || foundCombo._id}`
                  };
                  
                  // Cập nhật cache với nhiều keys
                  const normalizedName = normalizeText(comboData.name);
                  const originalName = comboData.name.toLowerCase().trim();
                  const nameWithoutTones = removeVietnameseTones(comboData.name).toLowerCase();
                  
                  setCombosCache(prev => {
                    const newCache = new Map(prev);
                    newCache.set(normalizedName, comboData);
                    newCache.set(originalName, comboData);
                    if (nameWithoutTones !== normalizedName) {
                      newCache.set(nameWithoutTones, comboData);
                    }
                    // Thêm với "combo" prefix
                    newCache.set(`combo ${normalizedName}`, comboData);
                    newCache.set(`combo ${originalName}`, comboData);
                    // Thêm với tên không có "combo" prefix
                    const nameWithoutCombo = comboData.name.replace(/^combo\s+/i, '').trim();
                    if (nameWithoutCombo !== comboData.name) {
                      const normalizedWithoutCombo = normalizeText(nameWithoutCombo);
                      newCache.set(normalizedWithoutCombo, comboData);
                    }
                    // Thêm với search term để dễ tìm hơn
                    const normalizedSearchTerm = normalizeText(searchTerm);
                    newCache.set(normalizedSearchTerm, comboData);
                    console.log('✅ [Combo Card] Updated cache with', newCache.size, 'entries');
                    return newCache;
                  });
            ...
            .catch(error => {
              console.error('❌ [Combo Card] Error searching combo from API:', error);
            });
        }
      });
    }
    ...
    // Re-check combo trong cache sau khi có thể đã được update
    let currentCombo = combo;
    if (!currentCombo) {
      // Thử tìm lại trong cache sau khi có thể đã được update
      currentCombo = findComboInCache(comboName) || 
                     (cleanComboName ? findComboInCache(cleanComboName) : null) ||
                     (cleanComboName ? findComboInCache(`combo ${cleanComboName}`) : null);
    }
    
    if (!currentCombo && (!comboName || comboName.length < 2)) {
      return null;
    }
  ...
  const markdownComponents: Components = {
    p: ({ children, ...props }) => {
      ...
      if (comboExtract && comboExtract.comboInfo) {
        ...
        if (comboCard) {
          // ✅ Render combo card bên ngoài <p> tag để tránh DOM nesting error (<div> không thể nằm trong <p>)
              ...
              >
                <div className="message-content">
                  <div className="message-text">
                    {/* ✅ Kiểm tra xem có cart summary không trước khi render markdown */}
                    {(() => {
                      if (message.isUser) {
                        return (
                          <ReactMarkdown>
                      ...
                      const cartSummary = parseCartSummaryFromFullText(message.text);
                      if (cartSummary && cartSummary.items.length > 0) {
                        // Nếu có cart summary, render card và phần text còn lại
                        const lines = message.text.split('\n');
                        const cartStartIndex = lines.findIndex(line => line.toLowerCase().includes('giỏ hàng'));
                        const cartEndIndex = lines.findIndex((line, idx) => idx > cartStartIndex && /tổng\s*(?:cộng)?\s*:?/i.test(line));
                        
                        // Lấy phần text trước và sau cart summary
                        const beforeCart = cartStartIndex > 0 ? lines.slice(0, cartStartIndex).join('\n').trim() : '';
                        const afterCart = cartEndIndex >= 0 && cartEndIndex < lines.length - 1 ? lines.slice(cartEndIndex + 1).join('\n').trim() : '';
                        ...
                        return (
                          <>
                            ...
                            {renderCartSummaryCard(cartSummary)}
                            {afterCart && (
                              <ReactMarkdown components={markdownComponents}>
                                {afterCart}
                              </ReactMarkdown>
                      ...
                      // Không có cart summary, render bình thường
                      return (
                        <ReactMarkdown components={markdownComponents}>
                          {message.text}
                        </ReactMarkdown>
